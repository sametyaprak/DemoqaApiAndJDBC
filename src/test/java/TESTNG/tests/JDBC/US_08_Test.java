...
public class US_08_Test {
    ...
    @AfterMethod
    public void after(){
        DatabaseConnectorX.closeConnection();
    }

    @Test
    public void tc0801() throws SQLException {
        resultset = DatabaseConnectorX.getResultSet(queryTC0801Select);
        ...
        Assert.assertEquals(eventType.size(),11);
            eventType.clear();
        DatabaseConnectorX.executeUpdateQuery(queryTC0801Update);
        resultset = DatabaseConnectorX.getResultSet(queryTC0801SelectAfterUpdate);
        while (resultset.next()){
            String queryData = resultset.getString("event_type");
    ...
    public void tc0801Fix(){
        ...
        DatabaseConnectorX.executeUpdateQueryFix(queryTC0801Fix);
        }
    @Test
    public void tc0802(){
        DatabaseConnectorX.executeDelete(quertTC0802Delete);
    }
    @Test
    public void tc0803() throws SQLException {
        resultset = DatabaseConnectorX.getResultSet(getQueryTC0803);
        while (resultset.next()){
